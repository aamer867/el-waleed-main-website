...
const Footer = () => {
  ...
  useEffect(() => {
    ...
    const fetchContact = async () => {
      try {
        const response = await fetch("/api/contact-us-section_en");
        const data = await response.json();
        setContactInfo(data.cards?.[0] ?? data);
      } catch (error) {
      ...
      <div className="container mx-auto px-8 py-10">
        <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
          ...
          <div className="flex flex-col items-center md:items-start">
            ...
            />
            <h1 className="text-xl font-bold">El-Waleed</h1>
          </div>

          {/* Column 2 - Contact */}
          <div className="flex flex-col items-center md:items-start">
            <h2 className="text-lg font-bold mb-3 text-main-red-color">Contact Us</h2>
          ...
          {/* Column 3 - Social */}
          <div className="flex flex-col items-center md:items-start">
            <h2 className="text-lg font-bold mb-3 text-main-red-color">Follow Us</h2>
        ...
        <div className="border-t border-gray-600 mt-8 pt-4 text-center">
          <p className="text-gray-400 text-sm">
            © {new Date().getFullYear()} El-Waleed — All Rights Reserved.
          </p>
        </div>
      </div>
    </footer>
